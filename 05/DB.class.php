...
class DB
{
  ...
  // Store the single instance of DB
     ...
     * returns: error
     */
    public function do_query($query, $vars=array(), $types=array())
    {
    ...
    //determine which type of query: select, insert, update, delete
      ...
      case "delete":
        ...
        if (substr_count($query, "?") != count($vars) || count($vars) != count($types)) {
            ...
            return $this->error;
        } elseif ($stmt = @$this->mysqli->prepare($query)) {
            if ($select) {
...
//        print_r($list);
     ...
     */
    public function fetch_array($row=0)
    {
        // retrieve row
     ...
     */
    public function get_col_names($tableName)
    {
        $cols_return = array();
     ...
     */
    public function get_col_info($tableName)
    {
        $cols_return = array();
        $cols = $this->mysqli->query("SHOW COLUMNS FROM $tableName");
     ...
     */
    public function get_num_records($tableName)
    {
        $numRecords = 0;
     ...
     */
    public function get_valid_table_names($pattern="")
    {
